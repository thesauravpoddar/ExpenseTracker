...
public class ExpCatServiceImpl implements ExpCatService{
    ...
    public ExpenseCategory getExpCatById(String userid, String expCatId) {
        ...
        return expCatRepo.getExpCatByExpenseCatId(expCatId , userid).orElseThrow(
                 () -> {
    ...
    public List<ExpenseCategory> listExpCatById(String userid) {
        return expCatRepo.findAllByUserId(userid);
    }

    @Override
    public ExpenseCategory updateExpenseCategory(String userid, String expCatId, String name) {
        var expenseCategory = expCatRepo.getExpCatByExpenseCatId(expCatId , userid).
                orElseThrow(
                        () -> {
                            return new CatNotfoundException(
                                    ErrorMessages.EXP_CAT_NOT_FOUND.getErrorMessage(),
       ...
       return expCatRepo.save(expenseCategory);
    }

    @Override
    public void deleteExpenseCategory(String userid, String expCatId) {
      var expenseCategory =  expCatRepo.getExpCatByExpenseCatId(expCatId , userid)
                .orElseThrow(
                        () ->
                        {
